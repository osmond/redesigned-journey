...
export async function POST(req: Request) {
  ...
  const plant = await prisma.plant.findUnique({ where: { id: parsed.data.plantId, userId: user.id } })
  ...
  if (parsed.data.type === 'WATER') await prisma.plant.update({ where: { id: plant.id, userId: user.id }, data: { lastWateredAt: new Date(), userId: user.id } })
  if (parsed.data.type === 'FERTILIZE') await prisma.plant.update({ where: { id: plant.id, userId: user.id }, data: { lastFertilizedAt: new Date(), userId: user.id } })
