...
export async function GET() {
  ...
  const rooms = await prisma.room.findMany({ where: { userId: user.id }, orderBy: { sortOrder: 'asc' } })
...
export async function POST(req: Request) {
  ...
  const room = await prisma.room.create({ data: { userId: user.id, name: parsed.data.name, sortOrder: count } })
